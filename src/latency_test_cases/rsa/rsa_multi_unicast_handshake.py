...
def run_one_trial():
    ...
    return sum_pk, sum_ct, finish-start

if __name__ == '__main__':
    for trial in range(1, NUM_TRIALS + 1):
        pk_latency, ct_latency, overall_latency = run_one_trial()

        with open("rsa_multiuni_pk_latency.txt", "a") as f:
            f.write(f"{trial},{pk_latency}\n")

        with open("rsa_multiuni_ct_latency.txt", "a") as f:
            f.write(f"{trial},{ct_latency}\n")

        with open("rsa_multiuni_overall_latency.txt", "a") as f:
